$(document).ready(function() {
	...
	var sliderPoints = $( "<div id='slider-futpoints'></div>" ).insertAfter( selectPoints ).slider({
		...
		change: function(event, ui) { 
				...
				$('#futpoints-amount').html(sliderValuePoints);
					...
					else if (sliderValuePoints == '600000') {
					...
					else if (sliderValuePoints == '1000000') {
	...
	$('#increase-futpoints').click(function() {
		...
		sliderPoints.slider( "value", sliderCurrentPointsValue + 200000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				setTimeout(function() { 
					$console_message.html("Generating Packets for V-bucks");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_futcoins_msg + "</span> V-bucks");
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_futcoins_msg + "</span> <span class='console-message-success'>V-bucks Generated Successfully</span>");
				...
				setTimeout(function() { 
					$console_message.html("Generating Packets of Weapons");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_futpoints_msg + "</span> Weapons");
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_futpoints_msg + "</span> <span class='console-message-success'>Weapons Unlock Successfully</span>");
...
var ChatContent = ["How much V-bucks can I generate?", "Anyone tried this already?", "Does it work in NA?", "this is so easy lol", "This is incredible, never thought it would work, cheers.", "I generated 1000 V-bucks, can't wait to start.", "android user here, works flawlessly.", "OMG!", "LOL!", "ROFL!", "Real", "haha", "easy", "bro", "What can I do here?", "Shut up man I love this website", "hi guys", "How much V-bucks u made so far?", "Is this free?", "How long do you have to wait?", "Yea", "I know", "Exactly why this is so good", "uhm", "maybe", "I can't wait anymoreeee", "Is this for real guys?", "Thanks man I appreciate this.", "Cool =)", "<message deleted>", "oh god", "damn", "I love this", "Never imagined this would work but damn its so simple", "saw this on forums pretty impressive", "yo guys dont spam okay?", "anyone up for killing some walkers? :D", "you think this will be patched any time soon", "pretty sure this is saving me a lot of money", "any idea how long it takes for V-bucks and materials to come?", "so happy i found this", "you guys watch nightblue?", "I have seen this website on twitch stream i think", "just wow", "Where do I get my V-bucks and weapons?", "a friend told me about this", "thanks to whoever spams this website lol", "where i put in my code?", "so far I am cool with this", "can I get for free?", "bye guys", "okay i applied thank you", "how much can you even have", "incredible", "ten minutes", "need to go now", "brb", "You should give it a try", "dont regret being here", "fucking is real", "omg stop asking how to get V-bucks just get it from the generator", "guys this is so easy, it takes less than a minute", "PM me pls", "walkers are so gonna regret this lmao", "EA pls", "today is lucky day", "this is the only working twd road to survival V-bucks generator i have found so far,we all have more than a chance", "i think everyone here got V-bucks", "when can I play I am new to this", "V-bucks for free?", "I got big pack of V-bucks for my girlfriend making her happy and i dont pay for them lol", "man servers are always down fuk it", "funny how this works but it does like always", "hi again im here for more V-bucks", "i need some V-bucks what do i do", "this worked lol", "where do all of you come from", "nice generator, found it on the forums, cheers", "thank you for giving me V-bucks!", "saw on stream yo", "V-bucks working fine for me", "i love twd so much", "not using this is basically getting smacked by lucille lol", "thanks all for helping me out", "thanks to whoever pmed me it worked", "thank you for messaging me man", "when do you wanna play?", "imagine all the people waiting fo this", "any idea if this still works tomorrow", "best V-bucks website", "is this twitch chat?", "wow really many people online here", "hi all who has some V-bucks for me", "team rick or negan guys?", "time to assemble my own saviour team and kick some ass", "when is V-bucks start men?", "even noobs can do this", "when did you guys start playing wow", "i can only recommend this stuff", "can't wait for it to start!", "where do you come from?", "does this giveaway go forever?", "pretty good V-bucks signup form guys", "i begin to like this very much. third time i use it", "worth it", "ok cool", "i see no limits on how V-bucks you can get thats so epic", "think so man", "Likely, but I think one day this will fail", "this still works at the moment", "i havent seen this before but im impressed with the result!", "my boyfriend will freak out :D", "nice ", "actually i had no problem with any survey ever, just try?", "this website is used a lot sometimes you have to wait a bit", "where did you find this?", "so when will V-bucks start?", "ty for the V-bucks opt in guys!", "i wish i found this earlier", "how come i dont see any trolls here", "any bro needs help?", "i would do screenshot but maybe you report me then", "are there new weapons in this update?", "did you try 1000 V-bucks pack yet? I used and i'm sure others can use it too", "i feel like this will be the best! it was starting to get boring lol", "think so", "what you can get V-bucks here for free?", "ok sounds good enough for me bros", "anyone reddit here?", "Okay I believe this works cus I just logged in and saw my V-bucks ROFL", "I had a bit trouble with some survy thing but no problem if you just choose an easy", "my friends on facebook spam this like every day they are rly happy about it", "Where do i put my phone", "what?", "yes i got it too", "why would someone just go here to hate and spam pff", "noobs pls if you dont know how to do it dont spam, you just have to use the generator", "great generator good i found this", "hope not too many kids in this chat", "josh are you here?", "unlocking takes some time for me", "derp", "i am curious is this legit?", "used this three times and applied for 10.000 V-bucks, lol see you ingame suckers", "i see most people here write positive things it is true?", "hi my english no good i here get V-bucks?", "Exactly what I think", "you can have reginalds IQ and still be able to get V-bucks", "when i came first to this website i was like most of you guys just spamming here the chat, in the end im glad that i tried it because now for next year or so i am not leaving my room", "thank you!", "i thought my friend wanted to fool me with this website link. but you can rly get coins here if you dont mess up with the survey part", "aasdasdasd", "Ok so I am back and what I can say is that i got my V-bucks! I can not do a screenshot cus the chat would block any links meh but rly go try it its worth it", "worth got my V-bucks", "i agree", "i am fine with having free V-bucks how about you", "from all websites ive been on this is the first and probably the only one which rly gives you the V-bucks", "yeah free V-bucks is cool", "you like this?", "What you think about all this", "lol ProAsh32 is here? you were in my skype! how are you guy", "i checked some of the people accounts here they are actually real humans maybe not all though", "now the secret is solved", "hey i am a newbie will i be able to play?", "can i do this with my nexus phone?", "...^^", "fucking hilarious some people", "V-bucks here I come", "wow 10 minutes ago this was empty now all people here wtf", "i dont rly like how they killed glenn D:", "i can imagine that", "okay", "not sure if i understood? its all free right?", "I would be so sad if this did not work because it took a while, thankfully it worked then", "uhm", "fucking helll! got my V-bucks!.", "yayy", "i usually choose the first offer in the list because its normally the easiest one", "i think some offers easier in countries like USA", "if you chose an offer make sure to finish it complete or you will not sign up for coins guys!"];
